...
const postProduct = (request, response, next) => {
  ...
  pool.query('INSERT INTO items (id, name, category, price) VALUES ($1, $2, $3, $4);', [id, name, category, price], (error, results) => {
    if (error) {
...
const deleteProductById = (request, response, next) => {
  ...
  pool.query('DELETE FROM items WHERE id = $1', [productId], (error, results) => {
    if (error) {
      throw error
    } 
    else if (productId) {
...
const updateProductById = (request, response, next) => {
  ...
  pool.query('UPDATE items SET name = $1, category = $2, price = $3 WHERE id = $4;', [name, category, price, productId], (error, results) => {
    if (error) {
      response.status(400).send('Invalid input')
      throw error
    } 
